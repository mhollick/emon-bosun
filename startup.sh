...
# it is kind of nice that as all used variables start with the same letter
# my text editor's autocomplete feature is really helpful.
...
readonly S_GRAPHITE_HOST=${GRAPHITE_HOST}
...
config_bosun () {
    ...
    local location=$4
    local email=$5
    sed -i "s/__SERVICE__/${service}/g" /etc/bosun.conf
    sed -i "s/__ENVIRONMENT__/${environment}/g" /etc/bosun.conf
    sed -i "s/__LOCATION__/${location}/g" /etc/bosun.conf
    sed -i "s/__EMAIL_ALERT_ADDRESS__/${email}/g" /etc/bosun.conf
}
...
config_bosun ${S_GRAPHITE_HOST} ${S_ENVIRONMENT} ${S_LOCATION} ${S_EMAIL_ALERT_ADDR}
